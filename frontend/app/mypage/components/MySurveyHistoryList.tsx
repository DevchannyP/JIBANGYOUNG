...
export default function MySurveyHistoryList() {
  ...
  const { data, isLoading, isError, isFetching, refetch } = useQuery<{
    ...
    queryKey: ["user", user?.id, "surveys", { page, size, sort }],
    // userId가 없으면 fetch X
    queryFn: () =>
      user?.id
  ...
  return (
    <section aria-labelledby="my-survey-history-title">
      ...
      <ul className={styles.mypageList}>
        {data.surveys.map((s: SurveyHistoryDto) => (
          <li
            key={s.id}
            className={styles.mypageListItem}
            tabIndex={0}
            aria-label={`${s.title} (${formatDate(s.participatedAt)})${s.isFavorite ? " 즐겨찾기" : ""}`}
          >
            {s.resultUrl ? (
              <a
                href={s.resultUrl}
            ...
            <span className={styles.mypageListTime}>
              {formatDate(s.participatedAt)}
            </span>
            {s.isFavorite && (
              <span className={styles.surveyStar} title="즐겨찾기">
                ★
              </span>
            )}
          </li>
