...
public class SimplePolicyTest {
    ...
    private void testPolicyDetailService() {
        ...
        for (Policy policy : testPolicies) {
            try {
                List<PolicyDetailDto> detailList = policyDetailService.getPolicyDetail(policy.getNO());
                if (!detailList.isEmpty()) {
                    ...
                    System.out.println("지원내용: " + (detail.getPlcy_sprt_cn() != null ? 
                        detail.getPlcy_sprt_cn().substring(0, Math.min(50, detail.getPlcy_sprt_cn().length())) + "..." : "없음"));
                    System.out.println("========================================\n");
                }
            } catch (Exception e) {
