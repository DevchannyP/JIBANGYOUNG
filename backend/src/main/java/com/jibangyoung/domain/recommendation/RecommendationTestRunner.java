...
public class RecommendationTestRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        recommendationAlgorithmService.generateRecommendations(testUserId, testResponseId);
        ...
        if (results.isEmpty()) {
            ...
            for (RecommendationResultDto dto : results) {
                System.out.println("---------------------------------------------------");
                System.out.printf("No: %d | RankGroup: %d | Rank: %d%n", dto.getNo(), dto.getRankGroup(),
                        dto.getRank());
                System.out.printf("Region Code: %d | Region Name: %s%n", dto.getRegionCode(), dto.getRegionName());
                System.out.println("Region Descriptions:");
                if (dto.getRegionDescription() != null && !dto.getRegionDescription().isEmpty()) {
                ...
                if (dto.getPolicies() != null && !dto.getPolicies().isEmpty()) {
                    dto.getPolicies().forEach(policy -> {
                        System.out.printf("  - 정책명: %s, 마감일: %s%n", policy.getPlcy_nm(), policy.getDeadline());
                    });
                } else {
